...
const _sfc_main = defineComponent({
  ...
  methods: {
    async updateStatus(id, e) {
      this.isLoading = true;
      await utils.changeStatus(route("supplier.status"), { id, status: e });
      this.isLoading = false;
    },
    async confirmDelete(index) {
      this.isLoading = true;
      await utils.deleteIndexDialog(route("sampling.destroy", this.projects[index].id), this.projects, index);
      this.isLoading = false;
    }
  },
  created() {
  ...
  ssrRenderList(_ctx.projects, (project, index) => {
    ...
    _push(ssrRenderComponent(_component_Link, {
      ...
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(`${ssrInterpolate(project.supplier.display_name)}`);
        } else {
          return [
            createTextVNode(toDisplayString(project.supplier.display_name), 1)
          ];
        }
      }),
      _: 2
    }, _parent));
    _push(`<span class="text-muted fw-semibold d-block fs-7"><i class="bi bi-people-fill me-2"></i>${ssrInterpolate(project.project_name)} (${ssrInterpolate(project.country.iso2)})</span></div>`);
    ...
    _push(`</div>`);
    if (_ctx.$page.props.user.role.role.slug != "user") {
      _push(`<div class="flex-1 text-end"><div class="form-switch form-check-solid d-block form-check-custom form-check-success"><input class="form-check-input h-20px w-30px" type="checkbox"${ssrIncludeBooleanAttr(((_a = project == null ? void 0 : project.supplier) == null ? void 0 : _a.status) == 1 ? true : false) ? " checked" : ""}><label class="form-check-label"> Status </label></div></div>`);
    } else {
      _push(`<!---->`);
    }
    if (_ctx.$page.props.user.role.role.slug != "user") {
