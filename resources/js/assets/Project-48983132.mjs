    ...
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        ...
        ssrRenderList(_ctx.surveys.data, (survey, index) => {
          _push2(`<tr${_scopeId}><td${_scopeId}>${ssrInterpolate(index + 1)}</td><td${_scopeId}>${ssrInterpolate(survey.project)}</td><td${_scopeId}>${ssrInterpolate(survey.user)}</td><td${_scopeId}>${ssrInterpolate(survey.starting_ip)}</td><td${_scopeId}>${ssrInterpolate(survey.end_ip)}</td><td${_scopeId}>${ssrInterpolate(survey.duration)}</td><td${_scopeId}>${ssrInterpolate(survey.created_at)}</td><td${_scopeId}>${ssrInterpolate(survey.client_browser)}</td><td${_scopeId}>`);
                  ...
                  createVNode("tbody", { class: "fw-semibold text-gray-600" }, [
                    (openBlock(true), createBlock(Fragment, null, renderList(_ctx.surveys.data, (survey, index) => {
                      return openBlock(), createBlock("tr", { key: index }, [
                        ...
                        createVNode("td", null, toDisplayString(survey.project), 1),
