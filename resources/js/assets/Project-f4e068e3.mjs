import { defineComponent, resolveComponent, withCtx, createTextVNode, createVNode, toDisplayString, withModifiers, openBlock, createBlock, withDirectives, vModelText, Fragment, renderList, createCommentVNode, useSSRContext } from "vue";
import { Link, Head } from "@inertiajs/inertia-vue3";
import Header from "./Header-c05ca8ff.mjs";
  ...
  _push(ssrRenderComponent(_component_app_layout, { title: _ctx.title }, {
    ...
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        ...
        _push2(`</div><button type="submit" class="btn btn-primary"${_scopeId}> Search </button><a target="_blank"${ssrRenderAttr("href", _ctx.route("user-project.report", { ..._ctx.$queryParams(), user_id: _ctx.user.data.id }))} class="btn btn-primary m-1"${_scopeId}><i class="bi bi-graph-down-arrow"${_scopeId}></i>Export Report</a></form></div><div class="card-body pt-0"${_scopeId}><div class="table-responsive"${_scopeId}><table class="table align-middle table-row-dashed fs-6 gy-5 text-center"${_scopeId}><thead${_scopeId}><tr class="text-gray-400 fw-bold fs-7 w-100 text-uppercase"${_scopeId}><!--[-->`);
        ...
        ssrRenderList(_ctx.surveys.data, (survey, index) => {
          _push2(`<tr${_scopeId}><td${_scopeId}>${ssrInterpolate(index + 1)}</td><td${_scopeId}>${ssrInterpolate(survey.project_id)}</td><td${_scopeId}>${ssrInterpolate(survey.user)}</td><td${_scopeId}>${ssrInterpolate(survey.starting_ip)}</td><td${_scopeId}>${ssrInterpolate(survey.end_ip)}</td><td${_scopeId}>${ssrInterpolate(survey.duration)}</td><td${_scopeId}>${ssrInterpolate(survey.created_at)}</td><td${_scopeId}>${ssrInterpolate(survey.client_browser)}</td><td${_scopeId}>`);
        ...
        if (_ctx.surveys.meta) {
          _push2(`<div class="row mb-5"${_scopeId}><div class="col-sm-12 d-flex align-items-center justify-content-between"${_scopeId}><span class="fw-bold text-gray-700"${_scopeId}> Showing ${ssrInterpolate(_ctx.surveys.meta.from)} to ${ssrInterpolate(_ctx.surveys.meta.to)} of ${ssrInterpolate(_ctx.surveys.meta.total)} entries </span>`);
        ...
        return [
          ...
          createVNode("div", { class: "card mb-5" }, [
            createVNode("div", { class: "card-header align-items-center gap-5" }, [
              ...
              createVNode("form", {
                ...
                createVNode("a", {
                  target: "_blank",
                  href: _ctx.route("user-project.report", { ..._ctx.$queryParams(), user_id: _ctx.user.data.id }),
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/User/Project.vue");
